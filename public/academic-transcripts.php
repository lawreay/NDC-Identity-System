...
<body class="bg-light"><?php require_once __DIR__ . '/partials/header.php'; ?><div class="container py-4">
    ...
    <?php $issuedTranscripts = array_filter($transcripts, static fn (array $transcript): bool => ($transcript['status'] ?? '') === 'issued'); ?>
    <?php if ($issuedTranscripts !== []): ?><div class="card shadow-sm mt-4"><div class="card-body"><h2 class="h6 mb-3">Issued transcript PDFs</h2><div class="d-flex flex-wrap gap-2"><?php foreach ($issuedTranscripts as $transcript): ?><a class="btn btn-outline-primary btn-sm" href="export-transcript.php?id=<?= (int) $transcript['id'] ?>"><i class="bi bi-file-earmark-pdf me-1" aria-hidden="true"></i><?= e((string) $transcript['transcript_number']) ?></a><?php endforeach; ?></div></div></div><?php endif; ?>
</div></body></html>
