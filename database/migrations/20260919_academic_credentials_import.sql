-- NDC Identity System: Academic and credential module import bundle
-- Prerequisite: the base application tables, including `students` and `users`, already exist.
-- This file is safe to import once into an existing NDC Identity System database.
...
INNER JOIN academic_terms AS t ON t.academic_year = '2026' AND t.name = 'FIRST TERM'
...
FROM students AS s
INNER JOIN academic_programmes AS p ON p.code = CASE TRIM(s.program)
